              ...
              {filtered.slice(0, 25).map(j => {
                ...
                return (
                  <tr key={j.id} className={`border-b border-[#F0F1F5] hover:bg-[#F8F9FC] transition-colors cursor-pointer ${selected.includes(j.id) ? 'bg-[#7C3AED]/5' : ''}`} onClick={() => { const s = j.id; setSelected(sel => sel.includes(s) ? sel.filter(x => x !== s) : [...sel, s]) }}>
                    <td className="py-2.5 px-4"><input type="checkbox" checked={selected.includes(j.id)} onChange={() => {}} onClick={e => e.stopPropagation()} className="accent-[#7C3AED]" /></td>
        ...
        {assistantOpen && (
          <motion.div initial={{ opacity: 0, y: 20, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.96 }} className="fixed bottom-24 right-6 z-50 w-[340px] rounded-3xl bg-white shadow-2xl border border-[#EBECF2] overflow-hidden">
            ...
            <div className="p-4 space-y-2">
              {[
                { l: 'Find empty publishing slots', a: () => { const empty = weekDays.filter(d => dayJobs(d).length === 0); toast.info(empty.length ? `Open slots: ${empty.map(d => d.toLocaleDateString('en', { weekday: 'short', month: 'short', day: 'numeric' })).join(', ')}` : 'No empty days this week — full schedule!') } },
                { l: 'Balance platform distribution', a: () => { const worst = Object.entries(platCounts).sort((a, b) => a[1] - b[1])[0]; toast.info(worst ? `Most underused: ${M[worst[0]]?.label} (${worst[1]} posts) — schedule more there.` : 'No posts yet') } },
