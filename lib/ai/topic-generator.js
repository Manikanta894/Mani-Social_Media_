...
IMPORTANT: Every platform caption must be unique and optimized for that specific platform's style. Include hashtags in each caption as specified. Do not leave any field empty. CRITICAL: when image analysis is provided, every caption must reference the actual image content (subject, visible text, mood, scene) — never generic advice unrelated to the image.

Respond with valid JSON only. No markdown fences. No explanation.`

  const raw = await callAi({ provider: textProvider, prompt, json: true, maxTokens: 2000 })
