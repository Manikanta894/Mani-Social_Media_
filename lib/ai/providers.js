...
export async function callAi({ provider, prompt, imageBase64, mimeType, images, json = false, maxTokens = 4096 }) {
...
async function callOpenAiCompatible({ apiKey, model, prompt, imageBase64, mimeType, images, json, maxTokens, baseUrl, label }) {
  ...
  if (!res.ok) {
    ...
    if (lower.includes('not support image') || lower.includes('image_url') || lower.includes('image input') || lower.includes('image.png'))
      throw new Error(`${label}: model "${model}" does not support image input. Switch to a vision-capable model (e.g. gpt-4o, llama-3.2-90b-vision-preview) in Settings → AI Providers.`)
...
export async function callVisionWithFallback(prompt, imageBase64, mimeType) {
  ...
  for (const type of visionOrder) {
    ...
    tried.push(type)
    try {
      return await callAi({ provider: p, prompt, imageBase64, mimeType, maxTokens: 2048 })
    } catch (e) {
  ...
  if (anyVision) {
    ...
    try { return await callAi({ provider: anyVision, prompt, imageBase64, mimeType, maxTokens: 2048 }) } catch {}
  }
  throw new Error(`All vision providers failed (tried: ${tried.join(', ') || 'none'}). No vision-capable provider available.`)
}
