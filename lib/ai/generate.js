...
import { callAi, pickTextProvider } from './providers'
...
async function runGeneration({ providers, visionProvider, textProvider, style, images: imgs, context, tone, jobId }) {
  ...
  if (imgs.length > 0 && visionProvider) {
    try {
      ...
      console.warn('[vision] skipped, generating text-only:', e.message)
      researchContext = ''
    }
  }
...
export async function generateBlogPost({ imageBase64, mimeType, context, styleId }) {
  ...
  const prompt = `You are a professional blog writer for INSIGHTS (insights.manikantar.in) — a premium publication like Harvard Business Review.
...
- 3-4 H2 subheadings with detailed body paragraphs
- One blockquote (>) highlighting a key insight from the article — not a generic quote
- Closing paragraph with a clear takeaway
- Total length: 800-1200 words
- Zero emojis anywhere in the entire article — this is a strict rule, never use emojis
- No Wikipedia references or links — never cite or link to Wikipedia
- Professional, editorial tone — no casual language, no emojis, no internet slang

OUTPUT FORMAT — respond with valid JSON only, no fences:
{
  "title": "Compelling blog title (under 70 chars)",
  "body_markdown": "Full article in GitHub-flavored markdown with ## headings, > blockquote, bullet lists",
...
Write as if for MIT Technology Review or Harvard Business Review. Professional, research-backed. NEVER use emojis. NEVER cite Wikipedia. Use real industry examples from 2025-2026.`
