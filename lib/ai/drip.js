...
export async function generateDripPosts(blogId, count = 4, spreadDays = 5) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const prompt = `You are a social media strategist. Extract one key insight from this blog post and write a ${platform} caption (max 200 words, engaging hook, 3-5 hashtags). Make it standalone — it should make sense without reading the full article. Blog title: "${blog.title}"
