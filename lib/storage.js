...
export const storage = {
  ...
  postDetails: {
    ...
    async upsert(row) {
      const sb = supabase()
      const { data, error } = await sb.from('post_details').upsert({
        ...
        checked_at: row.checked_at || new Date().toISOString(),
      }, { onConflict: 'job_id,platform' }).select().maybeSingle()
      throwIf(error)
      return data
    },
  },
  ...
  contentLibrary: {
    async list({ platform = null, limit = 500 } = {}) {
      const sb = supabase()
      let q = sb.from('content_library').select('*').order('published_at', { ascending: false }).limit(limit)
    ...
    async getByJob(jobId) {
      const sb = supabase()
      const { data, error } = await sb.from('content_library').select('*').eq('job_id', jobId)
      throwIf(error)
      return data || []
    },
    async upsert(row) {
      const sb = supabase()
      const { data: existing } = await sb.from('content_library').select('id').eq('platform', row.platform).eq('platform_post_id', row.postId).maybeSingle()
      if (existing) {
        const { data, error } = await sb.from('content_library').update({
          url: row.url, caption: row.caption, thumbnail_url: row.thumbnail, media_type: row.mediaType,
          source: row.source, job_id: row.jobId, published_at: row.publishedAt,
        ...
        throwIf(error)
        return { imported: false, data }
      }
      const { data, error } = await sb.from('content_library').insert({
