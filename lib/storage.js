...
export const storage = {
  ...
  topicQueue: {
    ...
    async nextPending() {
      // Priority: fresh pending topics first, then resume topics mid-pipeline
      // (status 'processing' with a saved article_data — steps expand/card).
      const rows = await tableList('topicQueue', r => r.status === 'pending' || r.status === 'processing')
      const fresh = rows.filter(r => r.status === 'pending').sort((a, b) => String(a.created_at).localeCompare(String(b.created_at)))
      if (fresh.length) return fresh[0]
      const resume = rows.filter(r => r.status === 'processing' && r.article_data).sort((a, b) => String(a.updated_at).localeCompare(String(b.updated_at)))
  ...
  driveQueue: {
    ...
    // FIFO ONLY: oldest eligible image (by queue_position, then discovered_at).
    // Never random. Excludes archived/locked/processing/used rows by status.
    async nextQueued() {
      const rows = await tableList('driveQueue', r => r.status === 'queued')
      if (!rows.length) return null
      return rows.sort((a, b) =>
        (Number(a.queue_position) || 0) - (Number(b.queue_position) || 0) ||
        String(a.discovered_at || '').localeCompare(String(b.discovered_at || ''))
      )[0] || null
    },
    async maxPosition() {
