...
export async function publishJob(job, options = {}) {
  ...
  // HARD APPROVAL GATE — nothing publishes unless the job was explicitly
  // approved/scheduled, or the caller opts in with explicit:true (manual
  // "Publish now" buttons). Silently publishing unapproved content is a bug.
  const explicit = options.explicit === true || options.forcePublish === true
  const allowed = explicit || ['approved', 'scheduled', 'publishing'].includes(job.status)
  if (!allowed) {
    console.warn(`[publish] BLOCKED job ${job.id} (status=${job.status}) — not approved`)
    return { status: 'blocked_pending_approval', blocked: true, results: [], error: 'Job not approved — publishing blocked' }
  }
  ...
  // Send publish result to Discord (primary) and Telegram (legacy fallback)
  try {
    const { notifyPublishResult } = await import('../discord/notify')
    await notifyPublishResult({ job, results })
  } catch (e) { console.warn('[publish] discord notify failed:', e.message) }
