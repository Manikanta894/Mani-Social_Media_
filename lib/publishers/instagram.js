...
export async function publishToInstagram({ caption, hashtags, imageUrl }) {
  ...
  if (!token || !igId) throw new Error('Instagram not configured (META_ACCESS_TOKEN / IG_BUSINESS_ACCOUNT_ID missing).')
  if (!imageUrl) throw new Error('Instagram requires an image URL — none provided on this job.')
