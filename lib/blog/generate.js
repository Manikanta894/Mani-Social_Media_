...
// Continue/expand a short article to the target length. Called by the next
// tick when the first pass came back under the quality threshold — fits the
// Hobby 60s budget because each pass is one AI call.
export async function continueArticle(article, { context = '', lastCategory = '' } = {}) {
  ...
  const prompt = `You are the AI Editor of INSIGHTS. An article was generated but it is only ${words} words — expand it to 900-1200 words while keeping the SAME structure, title, and voice.
...
Expand by:
- Adding 2-3 new H2 sections with real substance (examples, data points, case studies)
- Deepening each existing section with 1-2 more paragraphs
- Keeping the primary keyword ("${String(article?.primaryKeyword || article?.keywords || context).slice(0, 80)}") woven naturally — 1-2% density
- Preserving the existing FAQ or expanding it to 5 questions
- No fluff, no repetition, no AI-sounding filler — every new paragraph must add information

Respond with the COMPLETE updated article as JSON ONLY (same schema, full content):
{"title":"...","content":"...","excerpt":"...","slug":"...","metaDescription":"...","category":"...","tags":[...],"keywords":"...","faq":[...],"readingTime":...,"seoScore":...,"readabilityScore":...}`
  const raw = await callAi({ provider: textProvider, prompt, json: true, maxTokens: 3000, timeoutMs: 55000 })
  const cleaned = String(raw).trim().replace(/^```(?:json)?\s*/i, '').replace(/```\s*$/i, '').trim()
  const parsed = JSON.parse(cleaned)
  const newWords = String(parsed.content || '').split(/\s+/).filter(Boolean).length
  // Safety: only accept the expansion if it is meaningfully LONGER than the
  // original — a truncated AI response must never replace fuller content.
  if (newWords < 800 || newWords <= words) {
    throw new Error(`Expansion failed (${newWords} words vs original ${words})`)
  }
  return { ...article, ...parsed, expanded: true, expansionWords: words }
}
...
- SEO Title (under 70 chars, includes primary keyword) — MUST be a complete, compelling title based on the topic, NEVER a truncated/short version. E.g. for "How AI Agents Are Transforming Enterprise Workflows in 2026" use "How AI Agents Are Transforming Enterprise Workflows" — never just "AI Agents"
...
SEO KEYWORD RULES (critical — these power search ranking):
- The TOPIC itself is your primary keyword — use it VERBATIM (or its core phrase) in: the H1/H2 opening section, at least one H2 heading, the first 100 words, and the conclusion
- Define 3-4 secondary keywords derived from the topic (e.g. "AI agents enterprise", "agentic AI business") and naturally include each 2-3 times across the body
- Include 2-3 long-tail keyword phrases (question-form, e.g. "how do AI agents improve enterprise workflows") inside FAQ answers
- Keyword density 1-2% — natural, never stuffed, never robotic
- Use semantic/related terms (synonyms, LSI words) throughout for topical authority
...
OUTPUT FORMAT — respond with valid JSON only:
{
  ...
  "longTailKeywords": ["question-form keyword phrase 1", "question-form keyword phrase 2"],
  "internalLinks": ["suggested internal link anchor text/topic"],
  "externalLinks": ["suggested authoritative external domain"],
...
- Is the primary keyword used in the first 100 words AND in a heading AND in the conclusion? Yes/No
- Are secondary keywords naturally included 2-3x each? Yes/No
...
Respond with ONLY valid JSON. No markdown fences. No commentary.`
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      // Use nearly the full Hobby budget — the model needs time to finish
      const raw = await callAi({ provider: textProvider, prompt, json: true, maxTokens: 3000, timeoutMs: 55000 })
