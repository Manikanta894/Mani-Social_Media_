...
export async function runBlogTick() {
  ...
  if (nextTopic) {
    // Step-based: 'generating' -> generate article (one AI call, <60s),
    // then mark 'generated' so the NEXT tick sends the Discord card.
    ...
    try {
      ...
      await storage.audit.log('blog_generate', 'topic_queue', nextTopic.id, 'pending', 'used', { title: article.title, category: article.category, words: review.wordCount, score: review.score })
      ...
      return { source: 'topic', topic: nextTopic.topic, title: article.title, status, words: review.wordCount }
    } catch (e) {
