...
export async function runBlogTick() {
  ...
  if (nextTopic) {
    // Step-based: 'generating' -> generate article, then if short, expand on
    // the NEXT tick (one AI call per tick — fits the Hobby 60s budget).
    const isGenerating = nextTopic.status === 'processing'
    if (!isGenerating) {
      await storage.topicQueue.update(nextTopic.id, { status: 'processing' })
    }
    try {
      let article = nextTopic.article_data || await generateArticle({
      ...
      // Expansion pass: if the first pass was short, expand it now (this tick)
      if (!article.expanded && review.wordCount < 700) {
        try {
          const { continueArticle } = await import('./generate')
          article = await continueArticle(article, { context: nextTopic.topic, lastCategory: settings.last_category })
          article.quality = qualityReview(article)
          await storage.topicQueue.update(nextTopic.id, { article_data: article })
        } catch (e) {
          console.warn('[blog tick] article expansion failed (publishing as-is):', e.message)
        }
      }
      ...
      await storage.audit.log('blog_generate', 'topic_queue', nextTopic.id, 'pending', 'used', { title: article.title, category: article.category, words: review.wordCount, score: review.score, expanded: !!article.expanded })
      ...
      return { source: 'topic', topic: nextTopic.topic, title: article.title, status, words: review.wordCount, expanded: !!article.expanded }
    } catch (e) {
