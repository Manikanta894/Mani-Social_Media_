...
export async function runBlogTick() {
  ...
  await handleBlogApprovalTimeouts(sb, settings).catch(() => {})
  ...
  const { data: nextTopic } = await sb.from('topic_queue').select('*').eq('status', 'pending').order('created_at', { ascending: true }).limit(1).maybeSingle()
  
  if (nextTopic) {
    await sb.from('topic_queue').update({ status: 'processing' }).eq('id', nextTopic.id)
    try {
      ...
      await sb.from('topic_queue').update({ status: 'used', used_at: new Date().toISOString() }).eq('id', nextTopic.id)
      ...
      await sb.from('topic_queue').update({ status: 'failed', error: e.message }).eq('id', nextTopic.id)
      await logActivity('failed', null, { error: e.message, source: 'topic' })
      return { source: 'topic', error: e.message }
    }
  }
  ...
  try {
    ...
    const status = settings.approval_required ? 'pending_approval' : (settings.draft_mode ? 'draft' : 'approved')
    await sb.from('blog_queue').update({ status, article_data: article, generation_time: new Date().toISOString() }).eq('file_id', nextFile.file_id)
...
async function handleBlogApprovalTimeouts(sb, settings) {
  ...
  const { data: pending } = await sb.from('blog_queue').select('*').eq('status', 'pending_approval').limit(10)
  for (const item of pending || []) {
    if (action === 'skip') {
      await sb.from('blog_queue').update({ status: 'skipped' }).eq('file_id', item.file_id)
      ...
      await sb.from('blog_queue').update({ status: 'approved', approved_at: new Date().toISOString() }).eq('file_id', item.file_id)
      ...
      if (item.article_data) {
        try {
          ...
          await sb.from('blog_queue').update({ status: 'published', published_url: result.url, published_date: new Date().toISOString() }).eq('file_id', item.file_id)
        } catch {}
      }
    } else {
      ...
      await sb.from('blog_queue').update({ status: 'queued', queue_position: (maxRow?.queue_position || 0) + 1, article_data: null }).eq('file_id', item.file_id)
