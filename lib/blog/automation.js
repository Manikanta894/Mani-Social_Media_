...
export async function runBlogTick() {
  ...
  if (nextTopic) {
    ...
    try {
      ...
      try {
        ...
        await sendBlogToApproval({ article, fileId: null, status, fileName: nextTopic.topic })
  ...
  // Fallback: blog_queue (image-based) — RANDOM selection
    ...
    if (!settings.approval_required && !settings.draft_mode && settings.auto_publish) {
      try {
        ...
        await storage.imageLibrary.upsert({
          file_id: nextFile.file_id, name: nextFile.file_name || '', job_id: null, platform: 'blog',
