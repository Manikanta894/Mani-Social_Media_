...
export async function emitEvent({ type, source = 'system', platform = null, payload = {}, priority = null, notify = false }) {
  ...
  const { error } = await supabase().from('audit_log').insert({
    ...
    metadata: { ...payload, event: true, platform: event.platform, priority: event.priority, timestamp: event.timestamp },
  })
  if (error) console.warn('[events] emit failed:', error.message)

  // Optional notification routing (Telegram delivery channel)
  if (notify) {
    try {
      const s = await storage.settings.get()
      if (s.telegram_admin_chat_id) {
        const emoji = { breaking_news: '🔥', post_failed: '❌', workflow_failed: '⚠️', post_published: '✅', campaign_ready: '🎉' }[eventType] || '🔔'
        await sendMessage({ chatId: s.telegram_admin_chat_id, text: `${emoji} <b>${eventType.replace(/_/g, ' ')}</b> — ${source}${payload?.title ? ': ' + String(payload.title).slice(0, 120) : ''}` }).catch(() => {})
      }
    } catch {}
  }
  return event
...
export async function retryWebhookEvent(id) {
  ...
  const { data } = await supabase().from('audit_log').select('*').eq('entity_id', id).maybeSingle()
  if (!data) return { retried: false, reason: 'event not found' }
  await emitEvent({ type: data.action, source: data.entity, platform: data.metadata?.platform, payload: data.metadata, priority: data.metadata?.priority })
  return { retried: true }
}

export async function listEvents({ limit = 100, type = null, source = null, status = null } = {}) {
  let q = supabase().from('audit_log').select('*').order('performed_at', { ascending: false }).limit(limit)
  if (type) q = q.eq('action', type)
  if (source) q = q.eq('entity', source)
  if (status) q = q.eq('new_status', status)
...
export async function eventStats() {
  const { data, error } = await supabase().from('audit_log').select('action, new_status, performed_at').order('performed_at', { ascending: false }).limit(500)
  if (error) return { total: 0, today: 0, byType: {}, failed: 0, published: 0, lastHour: [] }
  const now = Date.now()
  ...
  for (const r of data || []) {
    ...
    if (now - ts < 3600 * 1000) lastHour.push({ type: t, source: null, ts: r.performed_at })
  }
