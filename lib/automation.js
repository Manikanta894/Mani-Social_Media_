...
function currentLocalHM(tz) {
  const now = new Date()
  // Compute offset in minutes from UTC for the target timezone
  let offsetMin = 0
  try {
    const tzNow = new Date(now.toLocaleString('en-US', { timeZone: tz }))
    const utcNow = new Date(now.toLocaleString('en-US', { timeZone: 'UTC' }))
    offsetMin = Math.round((tzNow.getTime() - utcNow.getTime()) / 60000)
  } catch { offsetMin = 0 }
...
export async function runTick() {
  ...
  try {
    ...
    // Try vision analysis (NVIDIA failover, fast timeout), fall back to generic topic
    let topic = 'Visual storytelling and creative content'
    try {
      const { analyzeImage } = await import('./ai/topic-generator')
      const vision = await Promise.race([
        analyzeImage(base64, mime_type),
        new Promise(res => setTimeout(() => res(null), 15000)),
      ])
      if (vision) topic = vision.slice(0, 500)
    } catch {}

    const { generateFullContent } = await import('./ai/topic-generator')
    const activeStyle = await storage.promptStyles.getActive()
    ...
    await sb.from('drive_queue').update({ status: settings.approval_required ? 'pending_approval' : 'approved', content_job_id: job.id, platform_content: content }).eq('file_id', nextFile.file_id)

    const s = await storage.settings.get()
    if (s.telegram_bot_token && s.telegram_admin_chat_id) {
      const { formatDraftMessage, buildJobKeyboard } = await import('./telegram/formatter')
      const msg = formatDraftMessage({ id: job.id, topic: topic.slice(0, 120), platform_posts: { linkedin: { caption: (content.PLATFORM_CAPTIONS?.linkedin || '').slice(0, 100) }, instagram: { caption: (content.PLATFORM_CAPTIONS?.instagram || '').slice(0, 100) }, facebook: { caption: (content.PLATFORM_CAPTIONS?.facebook || '').slice(0, 100) }, threads: { caption: (content.PLATFORM_CAPTIONS?.threads || '').slice(0, 100) } }, status: 'pending_approval', style_name: activeStyle?.name })
      await sendMessage({ chatId: s.telegram_admin_chat_id, text: msg, replyMarkup: buildJobKeyboard(job) }).catch(() => {})
    }

    return { processed: nextFile.file_id, job_id: job.id }
  } catch (e) {
