...
export const automation = {
  async get() {
    ...
    // Secret is derived â€” never persisted anywhere
...
export async function runTick() {
  ...
  // Skip background tasks â€” they keep the serverless function alive and cause timeouts
  ...
  // Publish any jobs whose scheduled_for time has arrived (publish-sweep)
  // This is the critical fix — previously scheduled jobs were NEVER published
  // because runTick() only processed the queue, not the scheduled_jobs list.
  try {
    ...
    if (due.length > 0) {
      const { publishSweep } = await import('./scheduler')
      const r = await publishSweep()
      storage.audit.log('publish_sweep', 'automation', 'tick', null, 'completed', { swept: r.swept || 0, results: (r.results || []).map(x => ({ job_id: x.job_id, status: x.status })) }).catch(() => {})
    }
  } catch (e) { console.warn('[tick] publish-sweep failed:', e.message) }

  // Also check for due news campaign scheduled assets
  try {
    const { resumeRunningCampaigns } = await import('./news/campaign')
    await resumeRunningCampaigns(9000).catch(() => {})
  } catch {}

  // Continuous Content Library sync (every 6h, tiny budget â€” never blocks the tick)
  try {
  ...
  // Automatic News Radar scan â€” moved to its own scheduled job (/api/automation/news)
  // to keep the tick fast and under the Vercel 60s function limit.

  // Sync intake uploads (Telegram/images) into the queue â€” self-healing pipeline
  try {
  ...
  // --- Self-heal: reset items stuck in 'processing' for 5+ min (Hobby 60s limit
  // kills functions mid-run, leaving rows locked) back to queued ---
  try {
    const staleCutoff = new Date(Date.now() - 5 * 60 * 1000).toISOString()
    const stuck = await storage.driveQueue.list({ status: 'processing' })
    for (const row of stuck) {
      const started = row.updated_at || row.discovered_at || ''
      if (started && started < staleCutoff && !row.tick_stage) {
        await storage.driveQueue.update(row.id, { status: 'queued', content_job_id: null, scheduled_time: null })
        await storage.audit.log('unlock', 'drive_queue', row.file_id, 'processing', 'queued', { reason: 'stale_lock' }).catch(() => {})
      }
    }
    // Requeue jobs stuck in processing older than 15 min (killed mid-pipeline)
    const jobsStale = new Date(Date.now() - 15 * 60 * 1000).toISOString()
    const procJobs = await storage.jobs.list({ status: 'processing' })
    for (const j of procJobs) {
      if (j.updated_at && j.updated_at < jobsStale) {
        await storage.jobs.update(j.id, { status: 'failed', warnings: [...(j.warnings || []), 'Timed out — requeued for retry'] })
        await storage.audit.log('timeout', 'content_job', j.id, 'processing', 'failed', { reason: 'hobby_60s_limit' }).catch(() => {})
      }
    }
  } catch (e) { console.warn('[tick] self-heal failed:', e.message) }
  ...
  const hasQueued = !!(await storage.csvTopics.nextUnused()) || !!(await storage.driveQueue.nextQueued())
  if (dueIdx === -1 && !hasQueued) return { skipped: `no slots in buffer window (now ${now.hour}:${now.minute}) and no queued items` }
  ...
  // When outside normal posting windows but queued items exist, bypass slot-cap check
  if (dueIdx >= 0 && created >= dueIdx + 1) return { skipped: `slot ${dueIdx + 1} already processed (${created}/${settings.posts_per_day})` }

  // Try CSV topic queue first (topic + image pairing, no vision needed).
  // Step-based like the image pipeline: 'content' step then 'card' step.
    ...
    try {
      ...
      if (pairImage) {
        try {
          ...
          const imageRefUrl = `${process.env.NEXT_PUBLIC_BASE_URL || `https://${process.env.VERCEL_PROJECT_PRODUCTION_URL || ''}`}/api/media/${pairImage.file_id}`
          imageRef = imageRefUrl; imageBase64Data = base64; mime = mime_type
          await storage.driveQueue.updateByFileId(pairImage.file_id, { status: 'processing' })
          await storage.imageLibrary.markUsed(pairImage.file_id, { jobId: null, name: pairImage.file_name || '' }).catch(() => {})
        } catch {}
      }
      ...
      try {
        ...
        for (const p of Object.keys(rawPosts)) {
          ...
          const q = await improveIfBelow({ platform: p, caption: rp.caption, hashtags: rp.hashtags || [], noAi: true })
      ...
      // Production rule: NOTHING publishes automatically — always wait for
      // Discord approval, regardless of the approval_required setting.
      ...
      await storage.csvTopics.update(nextCsvTopic.id, { status: 'pending_approval', image_path: imageRef })
      if (pairImage) { await storage.driveQueue.updateByFileId(pairImage.file_id, { status: 'pending_approval', content_job_id: job.id }) }
      await storage.audit.log('csv_generate', 'csv_topics', nextCsvTopic.id, 'processing', 'generated', { topic: nextCsvTopic.topic }).catch(() => {})

      // Send to Discord (primary) — fetch the FRESH job so the review card
      // shows the image, captions, and status (stale objects are incomplete)
      try {
        const fresh = await storage.jobs.get(job.id)
        if (fresh) {
          const { notifyDraft } = await import('./discord/notify')
          await notifyDraft(fresh, pairImage?.file_id || null)
        }
      } catch (e) { console.warn('[tick] discord draft notify failed:', e.message) }
  ...
  // =========================================================================
  // IMAGE PIPELINE — step-based, resumable across ticks.
  // Vercel Hobby kills functions at 60s; vision + generation exceed that, so
  // we do ONE step per tick and store progress (tick_step) on the queue row.
  // Steps: vision -> content -> card. Next cron (5 min) resumes automatically.
  // =========================================================================

  // Step 0: resume any image currently mid-pipeline
  const resumable = await storage.driveQueue.list({ status: 'processing' })
  const resumeRow = resumable.find(r => r.tick_step) || null
  const nextFile = resumeRow || await storage.driveQueue.nextQueued()
  if (!nextFile) return { skipped: 'no CSV topics and no queued photos' }

  const activeStyle = await storage.promptStyles.getActive()

  // --- STEP: VISION ---
  if (!nextFile.tick_step) {
    await storage.driveQueue.update(nextFile.id, { status: 'processing', tick_step: 'vision', updated_at: new Date().toISOString() })
    ...
    await storage.driveQueue.updateByFileId(nextFile.file_id, { content_job_id: job.id })
    ...
    try {
      ...
      const publicUrl = `${process.env.NEXT_PUBLIC_BASE_URL || `https://${process.env.VERCEL_PROJECT_PRODUCTION_URL || ''}`}/api/media/${nextFile.file_id}`
      await storage.jobs.update(job.id, { image_ref: publicUrl })

      await setStage(job.id, 'vision', { progress: 20, log: 'Analyzing image with NVIDIA vision' })
      let visionContext = ''
      try {
        visionContext = await Promise.race([
          analyzeImage(base64, mime_type, 40000),
          new Promise(res => setTimeout(() => res(null), 45000)),
        ])
        visionContext = visionContext || ''
      } catch (e) { await logStage(job.id, 'vision', 'Vision failed: ' + e.message) }
      await storage.jobs.update(job.id, { research_context: visionContext }).catch(() => {})
      await setStage(job.id, 'vision', { progress: 40, log: visionContext ? 'Vision complete' : 'Vision skipped (text-only)' })
      await storage.driveQueue.updateByFileId(nextFile.file_id, { tick_step: 'content' })
      return { step: 'vision', processed: nextFile.file_id, job_id: job.id, resuming: !!resumeRow }
    } catch (e) {
      await failJob(job.id, e.message, 'image')
      await storage.driveQueue.update(nextFile.id, { status: 'failed', error: e.message, tick_step: null })
      return { processed: nextFile.file_id, error: e.message, job_id: job.id }
    }
  }

  // --- STEP: CONTENT (generate captions for all platforms) ---
  if (nextFile.tick_step === 'content') {
    const jobId = nextFile.content_job_id
    if (!jobId) { await storage.driveQueue.update(nextFile.id, { status: 'queued', tick_step: null, content_job_id: null }); return { skipped: 'orphaned processing row — requeued' } }
    const job = await storage.jobs.get(jobId)
    if (!job) { await storage.driveQueue.update(nextFile.id, { status: 'queued', tick_step: null, content_job_id: null }); return { skipped: 'orphaned job — requeued' } }

    try {
      await setStage(job.id, 'content', { progress: 55, log: 'Generating platform-native content' })
      const topic = job.research_context
        ? `Base ALL of your content strictly on this image: ${job.research_context.slice(0, 700)}`
        : 'Visual storytelling and creative content'
      const { generateLeanContent } = await import('./ai/topic-generator')
      const content = await generateLeanContent({
        topic: topic,
        keywords: 'social media, engagement, professional insights',
        styleId: activeStyle?.id,
        visionContext: job.research_context || undefined,
      })
      ...
      await storage.jobs.update(job.id, { platform_posts: posts, warnings: [] })
      await storage.driveQueue.updateByFileId(nextFile.file_id, { tick_step: 'card', platform_content: content })
      return { step: 'content', processed: nextFile.file_id, job_id: jobId }
    } catch (e) {
      // Generation failed — leave row at 'content' so the next tick retries.
      // Only fail permanently after the row's own retry budget is exhausted.
      const retries = Number(nextFile.retry_count || 0)
      if (retries >= (nextFile.max_retries || 3)) {
        await failJob(job.id, e.message, 'content')
        await storage.driveQueue.update(nextFile.id, { status: 'failed', error: e.message, tick_step: null })
      } else {
        await storage.driveQueue.update(nextFile.id, { retry_count: retries + 1, error: e.message })
        await logStage(job.id, 'content', 'Retry ' + (retries + 1) + ': ' + e.message)
      }
      return { step: 'content', processed: nextFile.file_id, error: e.message, retry: retries + 1 }
    }
  }

  // --- STEP: CARD (finalize + send Discord approval card) ---
  if (nextFile.tick_step === 'card') {
    const jobId = nextFile.content_job_id
    const job = jobId ? await storage.jobs.get(jobId) : null
    if (!job || !job.platform_posts) {
      await storage.driveQueue.update(nextFile.id, { status: 'queued', tick_step: null, content_job_id: null })
      return { skipped: 'orphaned card step — requeued' }
    }
    try {
      const finalStatus = 'pending_approval'
      await setStage(job.id, 'approval', { progress: 90, log: 'Waiting for Discord approval', status: finalStatus })
      await storage.jobs.update(job.id, { status: finalStatus })
      await storage.driveQueue.update(nextFile.id, { status: finalStatus, tick_step: null, content_job_id: job.id })
      const fresh = await storage.jobs.get(job.id)
      if (fresh) {
        const { notifyDraft } = await import('./discord/notify')
        await notifyDraft(fresh, nextFile.file_id)
      }
      await setStage(job.id, 'approval', { progress: 95 })
      return { step: 'card', processed: nextFile.file_id, job_id: job.id, status: finalStatus }
    } catch (e) {
      await failJob(job.id, e.message, 'approval')
      await storage.driveQueue.update(nextFile.id, { status: 'failed', error: e.message, tick_step: null })
      return { processed: nextFile.file_id, error: e.message, job_id: job.id }
    }
  }

  return { skipped: 'unknown tick step' }
}

// Approve = mark READY only. Publishing happens ONLY via explicit
// "Publish Now" (Discord button) or the scheduler sweep for scheduled jobs.
export async function onApprove(job) {
  ...
  return { ready: true, note: 'Approved — job is READY. Publish via Publish Now.' }
}
...
export async function onPublishNow(job) {
  ...
  if (qr) {
    ...
    await storage.imageLibrary.upsert({
      ...
      name: qr.file_name || '',
      job_id: job.id,
      platform: publishedP.join(','),
      published_url: firstUrl,
      published_date: allOk ? new Date().toISOString() : null,
      status: allOk ? 'used' : 'failed',
    }).catch(() => {})
    await logActivity(allOk ? 'published' : 'failed', qr.file_id, job.id, { platforms: publishedP, failed: failedP, triggered_by: 'publish_now', url: firstUrl })
    const imageName = qr.file_name || ''
    if (allOk) {
      let archived = false
      try { const ar = await archiveIntakeFile(qr.file_id); archived = !!ar?.archived_to } catch {}
      let analyticsStarted = false
      try {
        const { startTracking } = await import('./analytics')
        await startTracking(job.id)
        analyticsStarted = true
      } catch (e) { console.warn('[automation] analytics start failed:', e.message) }
      try {
        const { sendPublishSuccess } = await import('./discord/approval')
        await sendPublishSuccess({ job, results: r.results, imageName, analyticsStarted, archived })
      } catch (e) { console.warn('[automation] success embed failed:', e.message) }
    } else {
      try {
        const { sendPublishFailure } = await import('./discord/approval')
        await sendPublishFailure({ job, results: r.results, imageName })
      } catch (e) { console.warn('[automation] failure embed failed:', e.message) }
    }
  }
  return r
}
...
async function handleApprovalReminders(settings) {
  ...
  for (const item of pending) {
    ...
    // scheduled_time stored as "HH:MM Asia/Kolkata" â€” parse minutes
    ...
    if (minsUntil > 0 && minsUntil <= 3) {
      ...
      if (!rem) {
        try {
          const { notifyApprovalReminder } = await import('./discord/notify')
          await notifyApprovalReminder({ fileId: item.file_id, fileName: item.file_name, scheduledTime: item.scheduled_time, minsUntil, action: settings.approval_timeout_action })
        } catch (e) { console.warn('[tick] discord reminder failed:', e.message) }
...
// --- Approval timeout: pending items past their slot â†’ configured behavior ---
async function handleApprovalTimeouts(settings) {
  ...
  for (const item of pending) {
    ...
    } else { // move_next â€” return to queue tail
      const newPos = (await storage.driveQueue.maxPosition()) + 1
