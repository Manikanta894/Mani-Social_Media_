...
export async function runTick() {
  ...
  // Fallback: skip image-only queue — users should use CSV topics for quality content
  if (!nextCsvTopic) {
    const { data: queueCount } = await sb.from('drive_queue').select('id', { count: 'exact', head: true }).eq('status', 'queued')
    if (queueCount > 0) return { skipped: `add topics to csv_topics table to generate content from ${queueCount} queued photos` }
    return { skipped: 'no CSV topics and no queued photos' }
  }
    ...
    await sb.from('drive_queue').update({
      ...
      ai_analysis: { vision_used: false, note: 'text-only generation' },
