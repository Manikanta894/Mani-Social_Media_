...
export async function runTick() {
  ...
  if (nextCsvTopic) {
    ...
    try {
      ...
      if (s.telegram_bot_token && (s.telegram_admin_chat_id || process.env.TELEGRAM_ADMIN_CHAT_ID)) {
        const chatId = s.telegram_admin_chat_id || process.env.TELEGRAM_ADMIN_CHAT_ID
        ...
        await sendMessage({ chatId, text: msg, replyMarkup: buildJobKeyboard(job) }).catch(() => {})
      }
  ...
  try {
    ...
    if (s.telegram_bot_token && (s.telegram_admin_chat_id || process.env.TELEGRAM_ADMIN_CHAT_ID)) {
      ...
      try {
        await tgPhoto({ chatId, photoUrl: publicUpload.url, caption: card, replyMarkup: kb })
      } catch (e) {
        console.warn('[tick] telegram photo send failed, falling back to text:', e.message)
        await tgSend({ chatId, text: card, replyMarkup: kb }).catch(() => {})
      }
    }
