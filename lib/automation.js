...
The content should feel natural, not robotic. Use the image details to craft relevant posts.`
    ...
    // Skip vision if provider is unreliable (free tier limits etc.)
    const providers2 = await storage.providers.list()
    const canDoVision = providers2.some(p => p.active_for_vision && p.api_key)
    const generateOpts = {
      context: aiContext,
    ...
    if (canDoVision) {
      generateOpts.imageBase64 = base64
      generateOpts.mimeType = mime_type
    }
    const result = await generateFromImage(generateOpts)
