...
export async function onApprove(job) {
  ...
  if (qr) {
    ...
    await logActivity(allOk ? 'published' : 'failed', qr.file_id, job.id, { platforms: publishedP, failed: failedP, url: firstUrl })
    const imageName = qr.file_name || ''
    if (allOk) {
      let archived = false
      try { const ar = await archiveIntakeFile(qr.file_id); archived = !!ar?.archived_to } catch {}
      // Auto-start analytics job immediately after successful publish
      ...
      // Failure embed → #failed-jobs. Image is NEVER archived on failure.
      try {
