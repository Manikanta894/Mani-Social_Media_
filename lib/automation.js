...
export async function runTick() {
  ...
  // Fallback: image-only pipeline (photo â†’ vision â†’ research â†’ content â†’ seo â†’ quality â†’ approval)
  ...
  try {
    ...
    // Service accounts have no Drive storage quota — cannot upload new files.
    // Serve the ORIGINAL Drive file directly through our public media proxy.
    ...
    const { pickTextProvider } = await import('./ai/providers')
    ...
    try {
      visionContext = await Promise.race([
        analyzeImage(base64, mime_type, 45000),
        new Promise(res => setTimeout(() => res(null), 50000)),
      ])
    ...
    // Persist vision context so the Discord review card can show real AI analysis
    ...
    await setStage(job.id, 'approval', {
      progress: 90,
      log: 'Waiting for Discord approval',
      status: finalStatus,
    })
    ...
    // Send to Discord — fetch the FRESH job (image, captions, status)
    try {
      const fresh = await storage.jobs.get(job.id)
      if (fresh) {
        const { notifyDraft } = await import('./discord/notify')
