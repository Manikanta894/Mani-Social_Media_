...
export async function runTick() {
  ...
  // Continuous Content Library sync (every 6h, small budget — never blocks the tick)
  try {
    const { maybeSyncLibrary } = await import('./content-library')
    const syncRes = await maybeSyncLibrary({ maxAgeMs: 6 * 60 * 60 * 1000, limit: 10, budgetMs: 15000 })
  ...
  // Automatic News Radar scan (every 15 min — brings breaking news into the queue)
  try {
    ...
    if (Date.now() - last > 15 * 60 * 1000) {
      ...
      const { runNewsCheck } = await import('./news')
      const r = await runNewsCheck()
      await sb.from('app_settings').upsert({ key: 'news_last_check', value: { at: new Date().toISOString() } }, { onConflict: 'key' })
      if (r.new > 0) {
        storage.audit.log('news', 'news_radar', 'tick', null, `${r.new} new item(s) detected`).catch(() => {})
        try {
      ...
      console.log(`[tick] news check done in ${Date.now() - started}ms (${r.new} new)`)
    }
  } catch (e) { console.warn('[tick] news check failed:', e.message) }
