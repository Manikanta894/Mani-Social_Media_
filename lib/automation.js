...
export async function runTick() {
  ...
  // Fallback: drive_queue (image → vision analysis → full content generation)
  const baseQuery = () => sb.from('drive_queue').select('*').eq('status', 'queued')
  ...
  try {
    ...
    // Step 1: Analyze image with vision failover chain
    const { analyzeImage, generateFullContent } = await import('./ai/topic-generator')
    const visionContext = await analyzeImage(base64, mime_type)

    const activeStyle = await storage.promptStyles.getActive()
    const topic = nextFile.file_name.replace(/\.[^/.]+$/, '').replace(/[-_]/g, ' ')

    // Step 2: Generate full content package using vision context
    ...
    await sb.from('drive_queue').update({
      ...
      ai_analysis: { vision_used: !!visionContext, vision_providers_tried: visionContext ? 'success' : 'all failed' },
      platform_content: content,
      ai_provider_used: textProvider.name,
    ...
    await storage.audit.log('generate', 'drive_queue', nextFile.file_id, 'processing', settings.approval_required ? 'pending_approval' : 'approved',
      { provider: textProvider.name, slot: dueIdx + 1 })
    await logActivity('ai_generated', nextFile.file_id, job.id, { slot: dueIdx + 1, provider: textProvider.name })

    try {
      const s = await storage.settings.get()
      if (s.telegram_bot_token && s.telegram_admin_chat_id) {
        const teleMsg = `📸 <b>New post generated</b>\n\n📁 ${topic}\n📊 ${settings.approval_required ? 'Pending approval' : 'Auto-approved'}\n\n💼 LinkedIn: ${(content.PLATFORM_CAPTIONS?.linkedin || '').slice(0, 100)}…\n📷 Instagram: ${(content.PLATFORM_CAPTIONS?.instagram || '').slice(0, 100)}…`
        await sendMessage({
          chatId: s.telegram_admin_chat_id,
          text: teleMsg,
          replyMarkup: {
            inline_keyboard: [[
              { text: '✅ Approve', callback_data: `appv:${job.id}` },
              { text: '🚀 Publish Now', callback_data: `pubn:${job.id}` },
              { text: '⏭ Skip', callback_data: `skip:${job.id}` },
              { text: '❌ Reject', callback_data: `rejt:${job.id}` },
            ]]
          },
        })
      }
