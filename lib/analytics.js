...
async function fetchOne(job) {
  ...
  for (const platform of PLATFORMS) {
    ...
    if (data) {
      const row = { job_id: job.id, platform, ...data, caption: (job.platform_posts?.[platform]?.caption || '').slice(0, 500), checked_at: new Date().toISOString() }
...
// Start an analytics job for ONE published post — called automatically right
// after a successful publish. Collects impressions/reach/likes/comments/shares/
// saves per platform and stores them in Google Sheets (postDetails + library).
export async function startTracking(jobId) {
  const job = await storage.jobs.get(jobId)
  if (!job) throw new Error(`Job ${jobId} not found`)
  const stats = await fetchOne(job)
  // Also record follower snapshots (growth tracking)
  try {
    for (const platform of PLATFORMS) {
      const pr = job.publish_results?.[platform]
      if (!pr?.post_id) continue
      await storage.contentLibrary.upsert({
        platform,
        postId: pr.post_id,
        url: pr.url || '',
        caption: (job.platform_posts?.[platform]?.caption || '').slice(0, 2000),
        mediaType: 'image',
        source: 'app',
        jobId: job.id,
        publishedAt: pr.published_at || new Date().toISOString(),
        likes: 0, comments: 0, shares: 0, saves: 0,
        impressions: 0, reach: 0, clicks: 0, profile_visits: 0,
        engagement_rate: 0,
      }).catch(() => {})
    }
  } catch (e) { console.warn('[analytics] library upsert failed:', e.message) }
  await storage.audit.log('start_tracking', 'analytics', jobId, null, 'started', { platforms: stats.map(s => s.platform) }).catch(() => {})
  return { tracked: stats.length, results: stats }
}

// Unified post list: Content Library (imported + app posts) merged with
// legacy post_details rows that have not been backfilled into the library.
export async function getPostAnalytics() {
  let library = []
  try { library = await getLibrary() } catch (e) { console.warn('[analytics] library unavailable:', e.message) }
  const all = await storage.postDetails.list()
  const jobs = await storage.jobs.list()
  const jobMap = Object.fromEntries(jobs.map(j => [j.id, j]))
  const libKeys = new Set(library.filter(r => r.job_id).map(r => `${r.job_id}:${r.platform}`))
  const legacy = all
    .filter(pd => !libKeys.has(`${pd.job_id}:${pd.platform}`))
    .map(pd => ({
      id: pd.id, platform: pd.platform, source: 'app', job_id: pd.job_id,
      url: jobMap[pd.job_id]?.published_url || null,
      thumbnail_url: jobMap[pd.job_id]?.image_ref || null,
      caption: pd.caption || jobMap[pd.job_id]?.platform_posts?.[pd.platform]?.caption || '',
      published_at: jobMap[pd.job_id]?.published_at || pd.checked_at,
      likes: pd.likes || 0, comments: pd.comments || 0, shares: pd.shares || 0, saves: pd.saves || 0,
      impressions: pd.impressions || 0, reach: pd.reach || 0, clicks: pd.clicks || 0, profile_visits: pd.profile_visits || 0,
      engagement_rate: pd.impressions > 0 ? Math.round(((pd.likes + pd.comments + pd.shares + pd.saves) / pd.impressions) * 1000) / 10 : 0,
      status: null, hashtags: [],
    }))
...
export async function getAggregatedStats() {
  ...
  const byPlatform = {}
  const totals = { impressions: 0, reach: 0, likes: 0, comments: 0, shares: 0, saves: 0, posts: 0, clicks: 0, profile_visits: 0 }
  ...
  return { totals, byPlatform, engagement, engagement_rate: totals.impressions > 0 ? ((engagement / totals.impressions) * 100).toFixed(2) : 0, raw: posts }
}

export async function getHashtagAnalytics() {
  const posts = await getPostAnalytics()
  const tagStats = {}
  const addTag = (tag, platform, impressions, engagement) => {
    ...
    tagStats[tag].total_impressions += impressions || 0
    tagStats[tag].total_engagement += engagement || 0
  }
  for (const post of posts) {
    const engagement = (post.likes || 0) + (post.comments || 0) + (post.shares || 0)
    const tags = post.hashtags || []
    for (const tag of tags) addTag(tag, post.platform, post.impressions, engagement)
  }
