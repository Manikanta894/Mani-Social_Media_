...
export async function handleInteraction(interaction) {
  ...
  console.log(`[discord] interaction type=${type} custom_id=${data.custom_id || data.name || '?'} id=${interaction.id}`)
...
async function handleComponent(interaction) {
  ...
  if (['appv', 'pubn', 'schd', 'edit', 'regn', 'skip', 'rejt'].includes(action)) {    const jobId = id2 || id1
    ...
    // ACK FIRST — Discord requires a response within 3s; job reads can be slow.
    // schd/edit respond with a MODAL as their ack (type 9), so they skip this.
    if (action !== 'schd' && action !== 'edit') {
      const jobAck = { appv: '✅ Approving…', pubn: '🚀 Publishing…', regn: '🔄 Regenerating…', skip: '⏭ Skipping…', rejt: '❌ Rejecting…' }
      await ackUpdate(interaction, jobAck[action] || '⏳ Working…')
    }
    const job = await storage.jobs.get(jobId)
    if (!job) {
      if (action !== 'schd' && action !== 'edit') {
        await editMessage({ channelId, messageId, embeds: [embed({ title: '❌ Job Not Found', description: jobId, color: 0xE74C3C })], components: [] }).catch(() => {})
      } else {
    ...
    if (action === 'appv') {
      try {
        ...
        const readyFileId = fileId || qr?.file_id || null
        ...
        await sendReadyCard({ channelId, messageId, job: updated, fileId: readyFileId })
      } catch (e) {
        await editMessage({ channelId, messageId, embeds: [embed({ title: '❌ Approve Failed', description: String(e.message).slice(0, 200), color: 0xE74C3C })], components: [] }).catch(() => {})
      }
      return
    }
    ...
    if (action === 'regn') {
      try {
        ...
        await editMessage({ channelId, messageId, embeds: [embed({ title: '❌ Regeneration Failed', description: String(e.message).slice(0, 200), color: 0xE74C3C })], components: [] }).catch(() => {})
      }
      return
    }
  ...
  // Prefix formats: "topic:<topicId>" (topic-sourced, no image) or "<fileId>:"
  if (action.startsWith('b') && ['bappv', 'bpubn', 'bschd', 'bedit', 'bregn', 'bskip', 'brejt', 'bnimg'].includes(action)) {
    const isTopic = id1 === 'topic'
    const topicId = isTopic ? id2 : null
    const fileId = isTopic ? null : id1
    const { blogApprove, blogPublishNow, blogReschedule, blogRegenerate, blogSkip, blogReject, topicApprove, topicPublishNow, topicReject, topicSkip, topicRegenerate } = await import('../blog/automation')

    // Topic-sourced card actions (SEO blogs from Topic Queue — no images)
    if (isTopic && topicId) {
      if (action === 'bappv') {
        await ackUpdate(interaction, '✅ Approving blog…')
        try {
          const r = await topicApprove(topicId)
          await storage.audit.log('blog_approve', 'topic_queue', topicId, 'used', 'published', { url: r?.url, triggered_by: 'discord' })
      ...
      if (action === 'bpubn') {
        ...
        try {
          const r = await topicPublishNow(topicId)
          await storage.audit.log('blog_publish', 'topic_queue', topicId, 'used', 'published', { url: r?.url, triggered_by: 'discord' })
          const emb = embed({ title: '🚀 Blog Published', description: r?.url ? `🔗 ${r.url}` : 'Published (no URL returned)', color: 0x2ECC71 })
          await editMessage({ channelId, messageId, embeds: [emb], components: [] }).catch(() => {})
        } catch (e) {
          await ackUpdate(interaction, `❌ ${String(e.message).slice(0, 200)}`, true)
      ...
      if (action === 'bregn') {
        await topicRegenerate(topicId)
        await ackUpdate(interaction, '🔄 Topic requeued — new article will generate on the next tick')
        return
      }
      if (action === 'bskip') {
        await topicSkip(topicId)
        await ackUpdate(interaction, '⏭ Topic skipped')
        return
      }
      if (action === 'brejt') {
        await topicReject(topicId)
        await ackUpdate(interaction, '❌ Topic rejected')
        return
      }
      await ackUpdate(interaction, '❌ Action not available for topic cards', true)
      return
    }

    if (!fileId) { await ackUpdate(interaction, '❌ Missing blog reference', true); return }
  ...
  if (action.startsWith('news_')) {
    ...
    const newsAck = { genall: '⚙️ Starting full campaign…', gen: '⚙️ Generating…', sched: '📅 Scheduling…', save: '🔖 Saving…', read: '🔗 Opening…', reanalyze: '🔄 Re-analyzing…', ignore: '❌ Ignoring…' }
    await ackUpdate(interaction, newsAck[newsAction] || '⏳ Working…')
    const item = await storage.newsPosts.get(newsId)
    if (!item) {
      await editMessage({ channelId, messageId, embeds: [embed({ title: '❌ News Item Not Found', description: newsId, color: 0xE74C3C })], components: [] }).catch(() => {})
      return
    }
  ...
  if (action.startsWith('li_')) {
    ...
    const liId = id1
    // ACK FIRST — Discord requires a response within 3s; DB reads can be slow
    const liAck = { appv: '✅ Approving…', edit: '✏️ Opening editor…', regn: '🔄 Regenerating…', sav: '🔖 Saving…', skip: '⏭ Skipping…', rejt: '❌ Rejecting…' }
    await ackUpdate(interaction, liAck[liAction] || '⏳ Working…')
    ...
    if (liAction === 'appv') {
      try {
        ...
        await editMessage({ channelId, messageId, embeds: [embed({ title: '❌ Approve Failed', description: String(e.message).slice(0, 300), color: 0xE74C3C })], components: [] }).catch(() => {})
      }
      return
    }
    if (liAction === 'edit') {
    ...
    if (liAction === 'regn') {
      try {
        ...
        await editMessage({ channelId, messageId, embeds: [embed({ title: '❌ Regenerate Failed', description: String(e.message).slice(0, 200), color: 0xE74C3C })], components: [] }).catch(() => {})
      }
      return
    }
    if (liAction === 'skip') {
