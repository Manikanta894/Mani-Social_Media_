...
import { buildContentPreviewEmbed, buildApprovalButtons, buildLinkedInEngagementEmbed, buildLinkedInEngagementButtons, sendPublishResult } from './approval'
...
async function handleComponent(interaction) {
  ...
  if (['appv', 'pubn', 'schd', 'edit', 'regn', 'skip', 'rejt'].includes(action)) {
    const jobId = id2 || id1
    ...
    if (action === 'appv') {
      try {
        ...
        await ackUpdate(interaction, '✅ Approved — will publish at scheduled time')
        // Update the message
        const emb = await buildContentPreviewEmbed(updated)
        emb.fields.push(field('✅ **Approved**', `Approved at ${new Date().toLocaleTimeString()}`, false))
        await editMessage({ channelId, messageId, embeds: [emb], components: [] }).catch(() => {})
        // Run auto-publish if enabled
        try {
          const r = await onApprove(updated)
          if (r?.results) await sendPublishResult({ job: updated, results: r.results }).catch(() => {})
        } catch (e) {
          await storage.audit.log('publish', 'content_job', jobId, 'approved', 'failed', { error: e.message, triggered_by: 'discord' })
        }
      } catch (e) {
    ...
    if (action === 'pubn') {
      ...
      try {
        ...
        await storage.audit.log('publish', 'content_job', jobId, job.status, 'published', { triggered_by: 'discord_postnow' })
        await sendPublishResult({ job, results: r.results }).catch(() => {})
        const emb = await buildContentPreviewEmbed(await storage.jobs.get(jobId))
        emb.fields.push(field('🚀 **Published**', `Published at ${new Date().toLocaleTimeString()}`, false))
        await editMessage({ channelId, messageId, embeds: [emb], components: [] }).catch(() => {})
      } catch (e) {
        await editMessage({
          channelId, messageId,
    ...
    if (action === 'schd') {
      ...
      await storage.audit.log('schedule', 'content_job', jobId, job.status, 'scheduled', { scheduled_for: scheduledFor, triggered_by: 'discord' })
      await ackUpdate(interaction, '📆 Scheduled for +1 hour')
      const emb = await buildContentPreviewEmbed(updated)
      emb.fields.push(field('📆 **Scheduled**', `For ${new Date(scheduledFor).toLocaleString()}`, false))
      await editMessage({ channelId, messageId, embeds: [emb], components: [] }).catch(() => {})
      return
    }
    ...
    // --- Edit ---
    if (action === 'edit') {
      await ackUpdate(interaction, '✏️ Reply with: `platform: new caption` — e.g. `linkedin: My new post text`')
      // Store pending edit state
      await storage.appState.set(`discord_pending_edit_${channelId}`, { jobId })
      return
    }
    ...
    // --- Reject ---
    if (action === 'rejt') {
      await ackUpdate(interaction, '❌ Rejected and archived')
      await onReject(job)
      const emb = await buildContentPreviewEmbed(await storage.jobs.get(jobId))
      emb.fields.push(field('❌ **Rejected**', 'Archived', false))
      await editMessage({ channelId, messageId, embeds: [emb], components: [] }).catch(() => {})
      return
    }
  }
  ...
  if (action.startsWith('li_')) {
    ...
    if (liAction === 'appv') {
      ...
      try {
        ...
        if (r.ok) {
          await editMessage({ channelId, messageId, embeds: [embed({ title: '✅ Comment Posted', description: `"${item.comment?.slice(0, 200)}"\n\n${item.url || ''}`, color: 0x2ECC71 })], components: [] }).catch(() => {})
        } else if (r.url_needed) {
          await editMessage({ channelId, messageId, embeds: [embed({ title: '✅ Approved — Manual Step Needed', description: `Couldn't auto-post. Copy and paste on LinkedIn:\n\n"${item.comment}"\n\n${item.url || ''}`, color: 0xF1C40F })], components: [] }).catch(() => {})
        } else {
...
async function handleModal(interaction) {
  ...
  if (customId.startsWith('modal_edit_')) {
    const jobId = customId.replace('modal_edit_', '')
