...
// --- LinkedIn Engagement Intelligence Center --------------------------------
...
export async function buildLinkedInEngagementEmbed(item) {
  ...
  const vis = item.estimated_visibility != null ? `${item.estimated_visibility}/100` : (item.visibility || '—')
  const spam = item.spam_risk != null ? `${item.spam_risk}%` : '—'
  const overall = item.overall_score != null ? `${item.overall_score}/100` : (item.opportunity != null ? `${item.opportunity}` : '—')
  const conv = item.conversation_potential != null ? `${item.conversation_potential}/100` : '—'
  const net = item.networking_score != null ? `${item.networking_score}/100` : '—'

  const fields = [
    field('👤 Author', item.author || item.author_estimate || 'LinkedIn discussion', true),
    field('🏢 Industry', item.industry || '—', true),
    field('🗂 Classification', item.classification || '—', true),
    field('🎯 Topic', item.topic || '—', false),
    field('🤖 AI Summary', String(item.ai_summary || item.summary || item.title || '').slice(0, 300), false),
    field('💡 Why this matters', String(item.why_engage || item.why || '').slice(0, 200), false),
    field('📈 Estimated Visibility', vis, true),
    field('🤝 Networking Score', net, true),
    field('🎯 Quality Score', `${item.quality || '—'}/100`, true),
    field('💬 Conversation Potential', conv, true),
    field('⚠️ Spam Risk', spam, true),
    field('📊 Overall', `**${overall}**`, true),
  ]

  const postText = String(item.title || '').slice(0, 200)
  const q = item.question_asked ? `\n\n❓ **Author asked:** "${String(item.question_asked).slice(0, 150)}"` : ''
  const intent = item.intent ? `\n🛠 **Intent:** ${item.intent}` : ''
  fields.push(field('📰 The Post', `${postText}${q}${intent}`, false))

  const strat = item.strategy ? `📌 **Strategy:** ${item.strategy}` : ''
  const sim = item.similarity != null ? ` · Similarity to past: ${item.similarity}%` : ''
  fields.push(field('💬 Suggested Comment', `${strat}${sim}\n\n"${String(item.comment || '').slice(0, 900)}"`, false))

  return embed({
    title: '💬 LinkedIn Engagement Opportunity',
    description: `**${item.title?.slice(0, 120) || 'LinkedIn post'}**\nAge: ${ageLabel} · ${item.topic || ''}`,
    color: 0x0A66C2,
    fields,
    footer: 'LinkedIn Engagement Intelligence Engine',
    timestamp: new Date().toISOString(),
    url: item.url || undefined,
  })
}

export function buildLinkedInEngagementButtons(id, url = null) {
  return [
    actionRow([
    ...
    actionRow([
      ...
      button({ label: '⏭ Skip', customId: `li_skip:${id}`, style: 2, emoji: '⏭' }),
      button({ label: '❌ Reject', customId: `li_rejt:${id}`, style: 4, emoji: '❌' }),
      ...(url ? [button({ label: '🔗 Open LinkedIn', url, style: 5, emoji: '🔗' })] : []),
    ]),
  ]
}
...
export async function sendLinkedInOpportunity(item) {
  ...
  const buttons = buildLinkedInEngagementButtons(item.id, item.url || null)
...
export function buildBlogApprovalButtons({ fileId = null, topicId = null }) {
  // Prefix encodes the reference: topic:<topicId> for topic-sourced cards,
  // otherwise the fileId for image-sourced cards.
  const prefix = topicId ? `topic:${topicId}` : fileId ? `${fileId}:` : ''
  return [
    actionRow([
...
export async function sendBlogToApproval({ article, fileId = null, topicId = null, status = 'pending_approval', fileName = '', imageUrl = '' }) {
  ...
  const buttons = buildBlogApprovalButtons({ fileId, topicId })
