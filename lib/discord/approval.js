...
export async function sendDraftToApproval(job, fileId = null) {
  const channelId = await getChannelId('approval-center')
  if (!channelId) return { skipped: 'approval-center channel not configured' }

  const previewEmbed = await buildContentPreviewEmbed(job)
  const buttons = buildApprovalButtons(job.id, fileId)
...
export async function sendPublishResult({ job, results }) {
  const channelId = await getChannelId('social-publishing')
  if (!channelId) return { skipped: 'social-publishing channel not configured' }
