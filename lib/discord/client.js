...
async function token() {
  // FAST PATH: env var — no Sheets read, critical for the 3s interaction ack
  if (process.env.DISCORD_BOT_TOKEN) return process.env.DISCORD_BOT_TOKEN
  // Cached settings lookup (60s) — avoids hammering Sheets on every call
  if (_tokenCache && Date.now() - _tokenCacheAt < 60 * 1000) return _tokenCache
