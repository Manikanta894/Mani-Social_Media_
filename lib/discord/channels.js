...
const CHANNEL_SPECS = {
  ...
  'social-approval':    { emoji: '📋', category: 'Publishing', topic: 'Social Auto approval queue — approve, edit, regenerate, reject' },
  'blog-approval':      { emoji: '📚', category: 'Publishing', topic: 'Blog Engine approval queue — SEO articles awaiting approval' },
  'published-log':      { emoji: '✅', category: 'Publishing', topic: 'Every successful publish — URL, platform, analytics status' },
  'failed-jobs':        { emoji: '❌', category: 'Publishing', topic: 'Failed publish attempts — errors and retry actions' },
  'automation-health':  { emoji: '🩺', category: 'Operations', topic: 'Automation health — ticks, queues, provider status' },
...
// Resolve a channel id by name, with legacy fallbacks so the studio works
// before/after a /setup re-run. Map: primary name → fallback names.
const CHANNEL_FALLBACKS = {
  'social-approval': ['approval-center'],
  'blog-approval': ['approval-center'],
  'published-log': ['social-publishing', 'announcements'],
  'failed-jobs': ['error-center', 'automation-logs'],
  'automation-health': ['system-health', 'dashboard'],
  'analytics': ['daily-reports'],
  'news-radar': [],
  'approval-center': ['social-approval'],
}
...
export async function getChannelId(name) {
  ...
  for (const fallback of CHANNEL_FALLBACKS[name] || []) {
    if (ids[fallback]) return ids[fallback]
  }
  return null
}
