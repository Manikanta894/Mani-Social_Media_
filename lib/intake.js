...
export async function uploadIntakeImage(base64, mimeType = 'image/jpeg', fileName) {
  ...
  const path = `${new Date().toISOString().slice(0, 10)}/${Date.now()}-${randomUUID().slice(0, 8)}-${clean}.${ext}`
...
export async function listIntakeFiles(limit = 500) {
  ...
  const sb = supabase()
  const files = []
  // Recurse the bucket — the API lists one folder at a time
  async function walk(prefix) {
    let offset = 0
    while (true) {
      const { data, error } = await sb.storage.from(INTAKE_BUCKET).list(prefix, {
        limit: 100,
        offset,
      ...
      for (const item of data) {
        if (item.id === null) {
          ...
          if (nextPrefix.startsWith(PROCESSED_PREFIX.replace(/\/$/, ''))) continue
          await walk(nextPrefix)
        } else {
          const path = prefix ? `${prefix}/${item.name}` : item.name
          if (path.startsWith(PROCESSED_PREFIX)) continue
          files.push({
            path,
            name: item.name,
...
export async function archiveIntakeFile(path) {
  await ensureBucket()
  const sb = supabase()
  const destination = PROCESSED_PREFIX + path.split('/').pop()
