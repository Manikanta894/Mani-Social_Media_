...
export async function archiveIntakeFile(fileId) {
  // Move the Drive file into the archived subfolder if configured
  const folder = process.env.GOOGLE_DRIVE_ARCHIVE_FOLDER_ID
  if (folder) {
    try {
      const file = await mediaStore.metadata(fileId)
      await mediaStore.upload(file.name, file.mimeType || 'image/jpeg', await mediaStore.download(fileId), folder)
      await mediaStore.remove(fileId)
      return { archived_to: folder }
    } catch {}
  }
  // Otherwise just mark the queue row archived
