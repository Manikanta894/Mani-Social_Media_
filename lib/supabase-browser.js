...
// Sync a session into the auth cookie. Accepts the session object directly
// (most reliable) or falls back to reading localStorage.
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed?.access_token) {
        setCookie(COOKIE_NAME, parsed.access_token)
        return true
      }
    }
  } catch {}
...
// Check whether the auth cookie is present (for redirect decisions)
