...
export function scoreOpportunity({ title = '', summary = '', analysis = null, topic = '' }) {
  const text = `${title} ${summary || ''}`.toLowerCase()
  const a = analysis || {}
  const base = Math.min(95, 55 + (title.length > 50 ? 8 : 0) + (/\?|!/.test(title) ? 5 : 0))

  const relevance = Math.min(100, base + (a.industry ? 6 : 0) + (a.topic ? 5 : 0))
  const conversation = Math.min(100, 55 + (a.question_asked ? 18 : 0) + (a.intent === 'debate' || a.intent === 'provoke' ? 15 : 0) + (/\?/.test(title) ? 10 : 0))
  const visibility = Math.min(100, 50 + (/\d/.test(title) ? 12 : 0) + (title.length > 70 ? 10 : 0) + (/\b(how|why|what|mistake|lesson|story|secret|future|trend)\b/.test(text) ? 12 : 0))
  const authority = Math.min(100, 50 + (a.industry ? 12 : 0) + (a.main_argument ? 10 : 0) + (a.takeaway ? 10 : 0))
  const originality = Math.min(100, 60 + (a.main_argument ? 15 : 0) + (summary.length > 200 ? 10 : 0))
  const trend = Math.min(100, 50 + (/\b(ai|2026|future|trend|new|breaking|surge|shift)\b/.test(text) ? 18 : 0) + (/\d{4}/.test(text) ? 8 : 0))
  const audience = Math.min(100, 55 + (a.target_audience ? 15 : 0) + (a.pain_point ? 12 : 0))
  // Spam risk: low for substantive, linkedin-origin posts; higher if title looks like a job post / ad
  ...
  const businessValue = Math.min(100, 50 + (a.industry ? 12 : 0) + (a.takeaway ? 12 : 0) + (/\b(roi|cost|revenue|growth|efficiency|market)\b/.test(text) ? 10 : 0))
  const networking = Math.min(100, 55 + (conversation >= 70 ? 15 : 0) + (a.question_asked ? 10 : 0) + (spamRisk < 10 ? 8 : 0))
  const learning = Math.min(100, 55 + (a.takeaway ? 15 : 0) + (a.main_argument ? 12 : 0) + (summary.length > 250 ? 8 : 0))
