...
function scorePost(title, summary, topic) {
  ...
  // Topic keywords: each word >2 chars must appear, OR any topic phrase matches
  ...
  // Synonym/related-term match (Google News rewrites titles, so literal
  // keyword matches are rare — accept related terms too)
  const RELATED = {
  ...
  const engagement = Math.min(99, 45 + (title.length > 60 ? 10 : 0) + (/\?|!/.test(title) ? 15 : 0) + (/\d/.test(title) ? 8 : 0) + (/how|why|lesson|learn|story|mistake|trend|future|breakthrough|surge|secret|mistake/.test(title.toLowerCase()) ? 12 : 0))
  // Match if: literal keyword hit, OR a related term hit, OR the title is
  // substantive (long + engaging) — Google News titles rarely contain the
  // exact topic string.
