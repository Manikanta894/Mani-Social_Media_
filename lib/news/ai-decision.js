...
async function findRelated(item) {
  const all = await storage.newsPosts.list()
  const related = (all || []).filter(r => r.id !== item.id).map(r => ({ id: r.id, title: r.title, source_name: r.source_name, published_at: r.published_at, status: r.status, ...{ overlap: titleOverlap(item.title, r.title) } })).filter(r => r.overlap >= 0.45)
...
export async function analyzeNewsItem(item, topics, learning = {}, sb = null) {
  ...
  corroboration = await findRelated(item).catch(() => corroboration)
...
export async function buildNewsCard(item, analysis) {
  ...
  const kb = {
    inline_keyboard: [
      [{ text: '⚙️ Generate All', callback_data: `nwca:${item.id}` }, { text: '📘 LinkedIn', callback_data: `nwgl:${item.id}` }, { text: '📝 Blog', callback_data: `nwbl:${item.id}` }],
...
export async function runNewsDecisionPipeline(limit = 6) {
  ...
  const items = (await storage.newsPosts.list('new')).slice(0, limit)
  ...
  for (const r of results) {
    ...
    if (sameIds.length > 0) {
      for (const dupId of sameIds) {
        ...
        await storage.newsPosts.update(dupId, { status: 'duplicate', duplicate_of: item.id })
        processedIds.add(dupId)
        merged++
      }
    }
    await storage.newsPosts.update(item.id, { ai_analysis: analysis })
    processedIds.add(item.id)

    const score = analysis.opportunity_score
    if (score >= 70) {
      await storage.newsPosts.update(item.id, { status: 'pending_approval', priority: analysis.priority })
      // Send to Discord (primary) and Telegram (legacy fallback)
      try {
        const { notifyNewsOpportunity } = await import('../discord/notify')
        await notifyNewsOpportunity(item, analysis)
      } catch (e) { console.warn('[news] discord notify failed:', e.message) }
      ...
      await storage.newsPosts.update(item.id, { status: 'ignored_by_ai', ai_ignored_reason: `opportunity ${score}/100 below 70` })
      ignored++
    }
  }
...
export async function sendEditorialBrief(type = 'morning') {
  ...
  const posts = await storage.newsPosts.list()
  ...
  const lines = type === 'morning'
    ? [
        `🌅 **Good Morning — Editorial Brief**`,
        ``,
        `Today's opportunities: **${todays.length}**`,
        `• Breaking / high priority: **${high}**`,
        `• Pending approval: **${pending}**`,
        `• Published: **${published}**`,
        `• AI ignored: **${ignored}**`,
        ...(best ? [``, `Best story: ${(best.title || '').slice(0, 80)} — AI ${best.ai_analysis?.opportunity_score || 0}/100`] : []),
      ]
    : [
        `🌙 **Evening Performance Brief**`,
        ``,
        `News scanned today: **${todays.length}**`,
        `• Approved: **${pending}**`,
        `• Ignored: **${ignored}**`,
        `• Published: **${published}**`,
        ``,
  ...
  try {
    const { logToChannel } = await import('../discord/dashboard')
    await logToChannel('daily-reports', {
      title: type === 'morning' ? '🌅 Morning Editorial Brief' : '🌙 Evening Performance Brief',
      description: lines.join('\n'),
      color: type === 'morning' ? 0xF1C40F : 0x9B59B6,
    })
  } catch (e) { console.warn('[news] discord brief failed:', e.message) }
