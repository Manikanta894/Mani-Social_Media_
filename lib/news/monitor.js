...
export async function checkNewsSources(budgetMs = 25000) {
  const sources = await storage.newsSources.listActive()
  const allItems = []
  const started = Date.now()
  for (const source of sources) {
    // Respect per-source check interval (minutes)
    if (source.check_interval && source.last_checked_at) {
      const last = new Date(source.last_checked_at).getTime()
      if (Date.now() - last < source.check_interval * 60 * 1000) continue
    }
    // Global time budget — never block the caller
...
export async function runNewsCheck(budgetMs = 25000) {
  const rawItems = await checkNewsSources(budgetMs)
  if (rawItems.length === 0) return { checked: 0, new: 0 }
  const enriched = await detectTrending(rawItems)
  let created = 0
  for (const item of enriched) {
    if (created >= 15) break // hard cap per run
