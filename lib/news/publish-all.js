...
export async function runNewsPublishAll() {
  try {
    ...
    console.error('[news-publish-all] failed:', e)
...
async function runInternal() {
  ...
  const pending = await storage.newsPosts.list('pending_approval')
  let best = null
  if (pending?.length) {
    ...
    const fresh = (await storage.newsPosts.list('new')).slice(0, 6)
    if (fresh?.length) {
      const topics = await getNewsTopics()
      const learning = await getLearning()
      const results = await Promise.all(fresh.slice(0, 3).map(async (it) => ({ it, a: await analyzeNewsItem(it, topics, learning) })))
      const scored = results.filter(r => r.a.opportunity_score >= 70).sort((x, y) => y.a.opportunity_score - x.a.opportunity_score)
      if (scored.length) {
        best = scored[0].it
        await storage.newsPosts.update(best.id, { status: 'pending_approval', ai_analysis: scored[0].a })
      }
    }
  }
  ...
  try {
    ...
    generated = withPosts.generated_posts || {}
  } catch (e) {
    return { ok: false, news_id: best.id, error: `Content generation failed: ${e.message}` }
  }
  ...
  await storage.newsPosts.update(best.id, {
