...
async function importLinkedIn({ limit = 25, out = {} }) {
  ...
  if (!token) return out
  let author = process.env.LINKEDIN_URN
  try {
    const { resolveLinkedInAuthor } = await import('./publishers/linkedin')
    author = await resolveLinkedInAuthor(token)
  } catch { author = process.env.LINKEDIN_URN }
