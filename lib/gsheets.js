...
async function gFetch(url, opts = {}) {
  ...
  const res = await fetch(url, { ...opts, headers, body: opts.body ? (typeof opts.body === 'string' ? opts.body : JSON.stringify(opts.body)) : undefined })
  if (res.status === 429) {
...
export async function driveMetadata(fileId) {
  const url = `https://www.googleapis.com/drive/v3/files/${fileId}?fields=id,name,mimeType,size&supportsAllDrives=true`
