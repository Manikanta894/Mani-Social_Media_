...
async function handleCallback(cq) {
  ...
  if (action.startsWith('blg_')) {
    ...
    if (!blogRow && blogAction !== 'reorder') {
      await answerCallbackQuery({ callbackQueryId: cq.id, text: 'Blog item not found', showAlert: true }); return
    }
    ...
    if (blogAction === 'rsch') {
      await answerCallbackQuery({ callbackQueryId: cq.id, text: '📅 Rescheduled' })
